...
public class ArenaManager {
    ...
    private static final Path ARENAS_FILE = CONFIG_DIR.resolve("arenas.json");
            ...
            try (Reader reader = Files.newBufferedReader(ARENAS_FILE)) {
                ...
                if (raw != null) {
                    for (String name : raw.keySet()) {
                        ...
                        if (p1 != null) cfg.pos1 = new BlockPos(p1.get("x").intValue(), p1.get("y").intValue(), p1.get("z").intValue());
                        if (p2 != null) cfg.pos2 = new BlockPos(p2.get("x").intValue(), p2.get("y").intValue(), p2.get("z").intValue());
                        arenas.put(name, cfg);
                    }
                }
            }
